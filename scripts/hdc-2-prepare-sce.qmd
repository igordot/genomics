...
Check if the files table corresponds to the imported flowSet

```{r}
unmatched_samples <- setdiff(files_tbl$file_name, rownames(pData(fs)))
unmatched_samples
if (length(unmatched_samples)) stop("files metadata table includes samples not in the flowSet")
```
...
# keep file_name as that is the link to the original flowSet
...
```{r save-coldata}
write_csv(distinct(as_tibble(colData(sce))), glue("{qc_dir}/sce-coldata.csv"))
```
