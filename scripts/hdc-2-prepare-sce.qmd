...
subtitle: "HDCyto workflow step 2"
...
# quarto render hdcyto-2-prepare-sce.qmd -P project_name:? -P arcsinh_cofactor:?
---

This script generates a SingleCellExperiment object with sample annotations.
The previous step (`hdcyto-1-import-fcs.qmd`) saves an unfiltered flowSet object.
...
The generated SingleCellExperiment object can then be used for CATALYST-based downstream analysis in the next step (`hdcyto-3-analyze-sce.qmd`).
...
quantile(assay(sce, "counts")[, sample(1:ncol(sce), 10000)])
```

```{r check-assay-exprs}
quantile(assay(sce, "exprs")[, sample(1:ncol(sce), 10000)])
```
