...
```{r plot-clusters}
for (clust in c("meta5", "meta8", "meta10", "meta15", "meta20")) {
  umap_clust <-
...
```{r}
# confirm that cluster_id and som100 are referring to the same clusters
if (identical(table(colData(sce)$cluster_id), table(cluster_ids(sce, "som100")))) {
  metadata_tbl <-
    left_join(
