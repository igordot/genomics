...
```{r settings-outputs}
phono_dir <- glue("{out_dir}/phenotypes")
exprs_dir <- glue("{out_dir}/expression")
clust_dir <- glue("{out_dir}/clusters")

dir.create(phono_dir, showWarnings = FALSE)
...
ggsave(glue("{qc_dir}/sce-exprs-density-raw.png"), density_plot, width = 16, height = 12)
```
...
for (p in names(colData(sce))) {
  if (n_distinct(colData(sce)[[p]]) < length(color_scheme)) {
    umap_pheno <-
      ...
      scale_color_manual(values = color_scheme)
    ggsave(glue("{phono_dir}/dr-umap-pheno-{p}.png"), umap_pheno, width = 10, height = 6)
  }
}
```
...
if (n_distinct(sce$sample_id) > 15) {
  for (sub_p in sort(unique(sce$patient_id))) {
    umap_pheno <-
      ...
      theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank()) +
      scale_color_manual(values = color_scheme)
    ggsave(glue("{phono_dir}/dr-umap-pheno-subset-sample_id-patient_id-{sub_p}.png"), umap_pheno, width = 10, height = 6)
  }
  for (sub_c in sort(unique(sce$condition))) {
    umap_pheno <-
      ...
      theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank()) +
      scale_color_manual(values = color_scheme)
    ggsave(glue("{phono_dir}/dr-umap-pheno-subset-sample_id-condition-{sub_c}.png"), umap_pheno, width = 10, height = 6)
  }
}
```
