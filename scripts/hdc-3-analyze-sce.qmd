...
subtitle: "HDCyto workflow step 3"
...
# quarto render hdcyto-3-analyze-sce.qmd -P project_name:?
---

This script performs CATALYST-based analysis, including dimensionality reduction and clustering.
It starts with a SingleCellExperiment object generated in the previous step (`hdcyto-2-prepare-sce.qmd`).
...
for (p in names(colData(sce))) {
  mds_plot <-
    pbMDS(sce, by = "sample_id", color_by = p, pal = color_scheme) +
    theme_classic() +
...
pb_plot <- plotPbExprs(sce, features = NULL) + scale_color_manual(values = color_scheme)
...
set.seed(99)
sce <- runDR(sce, dr = "UMAP", cells = 1000, n_neighbors = 50, features = NULL, min_dist = 0.3, n_epochs = 500)
sce
```
