---
title: "Perform CATALYST-based analysis"
subtitle: "High-dimensional cytometry workflow step 3"
...
params:
  ...
  # cells to subsample per sample for UMAP
  dr_cells_per_sample: 1000
# quarto render hdc-3-analyze-sce.qmd -P project_name:? -P dr_cells_per_sample:?
---

This script performs CATALYST-based analysis, including dimensionality reduction and clustering.
It starts with a SingleCellExperiment object generated in the previous step (`hdc-2-prepare-sce.qmd`).
...
if (!file.exists(sce_qs2)) stop("SingleCellExperiment does not exist: ", sce_qs2)
```

Define outputs

```{r settings-outputs}
pheno_dir <- glue("{out_dir}/phenotypes")
exprs_dir <- glue("{out_dir}/expression")
clust_dir <- glue("{out_dir}/clusters")

dir.create(pheno_dir, showWarnings = FALSE)
dir.create(exprs_dir, showWarnings = FALSE)
...
```{r import-sce}
sce <- qs2::qs_read(sce_qs2, validate_checksum = TRUE)
sce
```
...
Get the channels/features marked as useable

```{r}
if (ncol(rowData(sce)) < 3) stop("rowData not set")
channels_used <- rownames(sce)[rowData(sce)$use_channel == TRUE]
if (!length(channels_used)) stop("no useable channels")
```

Define the color scheme, accounting for many groups
...
for (p in names(colData(sce))) {
  mds_plot <-
    pbMDS(
      sce,
      by = "sample_id", features = channels_used,
      color_by = p, pal = color_scheme
    ) +
    theme_classic() +
    theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank(), strip.background = element_blank())
  ggsave(glue("{qc_dir}/dr-mds-pseudobulk-{p}.png"), mds_plot, width = 10, height = 6)
}
```

# Plot expression patterns

```{r plot-subset-channels}
sce_rand <- sce[channels_used, ]
```
...
if (length(samples_subset) > 20) {
  set.seed(99)
  samples_subset <- sort(sample(samples_subset, 15))
}
sce_rand <- sce_rand[, sce_rand$sample_id %in% samples_subset]
```
...
Generate a density plot for the original values

```{r plot-density-original}
if ("counts" %in% names(assays(sce_rand))) {
  exprs_tbl <- tidy_expression(sce = sce_rand, assay_name = "counts")
  # plot original values
  density_plot <-
    exprs_tbl |>
    plot_expression_density(values_col = "exprs")
  ggsave(glue("{qc_dir}/sce-exprs-density-raw.png"), density_plot, width = 20, height = 12)
  ...
  if (min(exprs_tbl$exprs) >= 0) {
    ...
    ggsave(glue("{qc_dir}/sce-exprs-density-log.png"), density_plot, width = 20, height = 12)
  }
}
```
...
density_plot <-
  ...
  plot_expression_density(values_col = "exprs")
ggsave(glue("{qc_dir}/sce-exprs-density-arcsinh.png"), density_plot, width = 20, height = 12)
```
...
pb_plot <- plotPbExprs(sce, features = channels_used) + scale_color_manual(values = color_scheme)
save_plot(glue("{exprs_dir}/exprs-markers-samples-boxplot.png"), pb_plot, base_width = 12, base_height = 6)
```
...
```{r plot-markers-samples-heatmap}
num_samples <- n_distinct(sce$sample_id)
plot_height <- 4 + (num_samples / 8)
plot_height <- round(plot_height, 1)
# hm_plot <- plotExprHeatmap(sce, k_pal = pal_igv()(51), m_pal = pal_igv()(51), scale = "last")
hm_plot <- plotExprHeatmap_ed(sce, features = channels_used, scale = "last")
png(glue("{exprs_dir}/exprs-markers-samples-heatmap.png"), width = 15, height = plot_height, units = "in", res = 300)
print(hm_plot)
dev.off()
```
...
agg_exprs(sce, scale = "never") |> write_csv(glue("{exprs_dir}/exprs-samples-median.csv"))
# aggregate assay data first and scale subsequently (range of each marker will be 0-1)
agg_exprs(sce, scale = "last") |> write_csv(glue("{exprs_dir}/exprs-samples-median-scaled.csv"))
...
set.seed(99)
sce <- runDR(sce, dr = "UMAP", cells = params$dr_cells_per_sample, n_neighbors = 50, features = channels_used, min_dist = 0.3, n_epochs = 500)
sce
```
...
```{r}
dr_cells <- reducedDims(sce)$UMAP[, 1]
dr_cells <- dr_cells[!is.na(dr_cells)]
dr_cells <- names(dr_cells)
length(dr_cells)
```
...
```{r umap-phenotypes}
for (p in names(colData(sce))) {
  if (n_distinct(colData(sce)[[p]]) < length(color_scheme)) {
    umap_pheno <-
      plotDR(sce_rand, "UMAP", color_by = p) +
      theme_classic() +
      theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank()) +
      scale_color_manual(values = color_scheme)
    ggsave(glue("{pheno_dir}/dr-umap-pheno-{p}.png"), umap_pheno, width = 10, height = 6)
  }
}
```
...
if (n_distinct(sce$sample_id) > 15) {
  for (sub_p in sort(unique(sce$patient_id))) {
    umap_pheno <-
      ...
      theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank()) +
      scale_color_manual(values = color_scheme)
    ggsave(glue("{pheno_dir}/dr-umap-pheno-subset-sample_id-patient_id-{sub_p}.png"), umap_pheno, width = 10, height = 6)
  }
  for (sub_c in sort(unique(sce$condition))) {
    umap_pheno <-
      ...
      theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank()) +
      scale_color_manual(values = color_scheme)
    ggsave(glue("{pheno_dir}/dr-umap-pheno-subset-sample_id-condition-{sub_c}.png"), umap_pheno, width = 10, height = 6)
  }
}
```
...
for (m in sort(rownames(sce))) {
  marker_plot <-
    ...
    theme_cowplot() +
    theme(aspect.ratio = 1, axis.text = element_blank(), axis.ticks = element_blank(), strip.background = element_blank())
  ggsave(glue("{exprs_dir}/dr-umap-exprs-{m}.png"), marker_plot, width = 8, height = 6)
}
```
...
```{r cluster}
sce <- cluster(sce, features = channels_used, xdim = 15, ydim = 15, maxK = 50, seed = 99)
```
...
```{r plot-clusters}
for (clust in c("meta5", "meta8", "meta10", "meta15", "meta20", "meta25", "meta30", "meta35", "meta40", "meta50")) {
  umap_clust <-
  ...
  for (p in names(colData(sce))) {
    if (n_distinct(colData(sce)[[p]]) < 25) {
      plot_bar <- plotAbundances(sce, k = clust, by = "sample_id", group_by = p, k_pal = pal_igv()(51))
      ggsave(glue("{clust_dir}/abundance-bar-{clust}-{p}.png"), plot_bar, width = 12, height = 9)

      plot_box <- plotAbundances(sce, k = clust, by = "cluster_id", group_by = p, k_pal = pal_igv()(51))
      ggsave(glue("{clust_dir}/abundance-box-{clust}-{p}.png"), plot_box, width = 12, height = 6)
    }
  }
}
```

Generate a complete metadata table (meta-clusters are not stored in colData)
