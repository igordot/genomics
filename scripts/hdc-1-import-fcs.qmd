---
title: "Import FCS files and generate a flowSet object"
subtitle: "High-dimensional cytometry workflow step 1"
...
# quarto render hdcyto-1-import-fcs.qmd -P project_name:? -P fcs_dir:? -P max_cells_per_sample:?
---
...
It generates `input/metadata-files.csv` and `input/metadata-channels.csv` metadata tables which should be manually edited to clean up labels, define sample groups, and specify the relevant samples and markers/antibodies used for the next step (`hdc-2-prepare-sce.qmd`).
...
`CATALYST::guessPanel()` guesses the marker class based on the channel name and outputs CATALYST-compatible column names (some columns may need to be coerced to character from list)
...
```{r save-flowSet}
qs2::qs_save(object = fs, file = fs_qs2)
unlink(nfs_cdf)
```
...
Add `desc0` column to the channels table (added by CATALYST if `desc` has underscores)

```{r check-desc0}
if (!"desc0" %in% names(channels_df)) {
  channels_df$desc0 <- channels_df$desc
}
```

Generate a ggplot-friendly expression table with extreme outliers removed
...
dens_plot <-
  ...
  scale_color_igv()
ggsave(glue("{qc_dir}/flowset-exprs-density-raw.png"), dens_plot, width = 20, height = 12)
```
...
if (min(exprs_tbl$exprs) >= 0) {
  dens_plot <-
    ...
    scale_color_igv()
  ggsave(glue("{qc_dir}/flowset-exprs-density-log.png"), dens_plot, width = 20, height = 12)
} else {
  warning("negative expression values present")
}
```
