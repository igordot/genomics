...
"
Analysis of 10x Genomics Chromium single cell RNA-seq data using Seurat (version 1.4) starting with Cell Ranger output.
...
create_seurat_obj = function(counts_matrix, proj_name = NULL, sample_dir = NULL) {
  ...
  counts_matrix = counts_matrix[rowSums(counts_matrix) >= 5, ]
  ...
  s_obj = new("seurat", raw.data = counts_matrix)

  message("\n\n ========== seurat setup ========== \n\n")

  # keep all genes expressed in >= 3 cells, keep all cells with >= 200 genes
  # perform log-normalization, first scaling each cell to a total of 10000 molecules
  # Setup(pbmc33k, min.cells = 3, min.genes = 200, project = "10X_PBMC", names.field = 2, names.delim = "\\-")

  if (is.null(proj_name)) {

    # if name is not set, then it's a manually merged counts matrix
    s_obj = Setup(s_obj, min.cells = 10, min.genes = 300, do.logNormalize = TRUE, total.expr = 10000,
                  project = "proj", save.raw = FALSE, names.field = 1, names.delim = ":")
    ...
    s_obj = Setup(s_obj, min.cells = 10, min.genes = 300, do.logNormalize = TRUE, total.expr = 10000,
                  project = proj_name, save.raw = FALSE, names.field = 2, names.delim = "-")
    ...
    # change s_obj@data.info$orig.ident sample identities from numbers to names
    s_obj@data.info$orig.ident = factor(sample_sheet[s_obj@data.info$orig.ident, 1])
    # set s_obj@ident to the new s_obj@data.info$orig.ident
  ...
  percent_mt = colSums(expm1(s_obj@data[mt_genes, ])) / colSums(expm1(s_obj@data))
  percent_mt = round(percent_mt * 100, digits = 3)

  # add columns to object@data.info, and is a great place to stash QC stats
  s_obj = AddMetaData(s_obj, percent_mt, "percent_mito")

  message("\n\n ========== nGene/nUMI/percent_mito plots ========== \n\n")

  png("qc.distribution.unfiltered.png", res = 200, width = 10, height = 5, units = "in")
  VlnPlot(s_obj, c("nGene", "nUMI", "percent_mito"), nCol = 3, size.use = 0.2, size.title.use = 12, cols.use = color_scheme)
  dev.off()
  ...
  par(mfrow = c(1, 2))
  GenePlot(s_obj, gene1 = "nUMI", gene2 = "percent_mito", cex.use = 1.2, col.use = color_scheme)
  ...
  s_obj@data.info$nGene %>% quantile(low_quantiles) %>% round(digits = 1) %>% print()
  message(" ")
  message("nGene high percentiles:")
  s_obj@data.info$nGene %>% quantile(high_quantiles) %>% round(digits = 1) %>% print()
  message(" ")
  message("percent_mito high percentiles:")
  s_obj@data.info$percent_mito %>% quantile(high_quantiles) %>% round(digits = 1) %>% print()
  message(" ")
...
filter_data = function(seurat_obj, min_genes = NULL, max_genes = NULL, max_mt = 10) {
  ...
  if (!length(min_genes)) min_genes = s_obj@data.info$nGene %>% quantile(0.02, names = FALSE) %>% round(-1)
  if (!length(max_genes)) max_genes = s_obj@data.info$nGene %>% quantile(0.98, names = FALSE) %>% round(-1)
  ...
  s_obj = SubsetData(s_obj, subset.name = "nGene", accept.low = min_genes)
  s_obj = SubsetData(s_obj, subset.name = "nGene", accept.high = max_genes)
  s_obj = SubsetData(s_obj, subset.name = "percent_mito", accept.high = max_mt)

  message("filtered genes: ", nrow(s_obj@data))
  message("filtered cells: ", ncol(s_obj@data))

  png("qc.distribution.filtered.png", res = 200, width = 10, height = 5, units = "in")
  VlnPlot(s_obj, c("nGene", "nUMI", "percent_mito"), nCol = 3,
          size.use = 0.2, size.title.use = 12, cols.use = color_scheme)
  dev.off()
...
calculate_variance = function(seurat_obj) {
  ...
  # this overwrites s_obj@scale.data, so you can set do.scale=F and do.center=F in the original object to save time
  s_obj = RegressOut(s_obj, latent.vars = c("nUMI", "percent_mito"))

  message("\n\n ========== Seurat::MeanVarPlot() ========== \n\n")

  # detection of variable genes across the single cells
  # typical parameter settings for UMI data that is normalized to a total of 1e4 molecules
  png("variance.meanvar.png", res = 200, width = 8, height = 5, units = "in")
    # s_obj = MeanVarPlot(s_obj, fxn.x = expMean, fxn.y = logVarDivMean, x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5, do.contour = FALSE)
    # more stringent bottom cutoff (still lower than default 0.1)
    s_obj = MeanVarPlot(s_obj, fxn.x = expMean, fxn.y = logVarDivMean,
                        x.low.cutoff = 0.05, x.high.cutoff = 3, y.cutoff = 0.5, do.contour = FALSE)
  dev.off()
  ...
  s_obj = PCA(s_obj, pc.genes = s_obj@var.genes, do.print = FALSE, pcs.print = 3, genes.print = 5)

  # ProjectPCA scores each gene in the dataset based on their correlation with the calculated components
  # it can be used to identify markers that are strongly correlated with cellular heterogeneity, but may not have passed through variable gene selection
  s_obj = ProjectPCA(s_obj)
  ...
  PCHeatmap(s_obj, pc.use = 1:15, cells.use = 500, do.balanced = TRUE, label.columns = FALSE, use.full = FALSE)
  dev.off()
  ...
  if (ncol(s_obj@data) < 10000) {
    ...
    # resampling test inspired by the jackStraw procedure (can take a long time)
    s_obj = JackStraw(s_obj, num.pc = 40, num.replicate = 100, do.print = FALSE)
...
calculate_clusters = function(seurat_obj, num_pcs) {
  ...
  message("initial data.info: ", paste(colnames(s_obj@data.info), collapse = ", "))
  ...
  s_obj = FindClusters(s_obj, pc.use = 1:num_pcs, resolution = seq(0.1, 2.1, 0.2),
                       save.SNN = TRUE, print.output = FALSE)

  message("new data.info: ", paste(colnames(s_obj@data.info), collapse = ", "))
  ...
  ggsave(glue("tsne.pcs{num_pcs}.original.png"), plot = plot_tsne, width = 7, height = 6, units = "in")
  ggsave(glue("tsne.pcs{num_pcs}.original.pdf"), plot = plot_tsne, width = 7, height = 6, units = "in")

  # plot clusters for calculated resolutions
  res_cols = grep("^res.", colnames(s_obj@data.info), value = TRUE)
  for (res in res_cols) {
    ...
    # plot file name
    res_str = gsub("\\.", "", res)
    num_clusters = s_obj@data.info[, res] %>% unique() %>% length()
...
set_resolution = function(seurat_obj, resolution) {
  ...
  if (res_col %in% colnames(s_obj@data.info)) {
...
plot_genes = function(seurat_obj, genes, name) {
  ...
  vln_filename = paste0(name, ".violin.png")
  message("save plot: ", vln_filename)
  png(vln_filename, res = 200, width = 15, height = 8, units = "in")
    VlnPlot(seurat_obj, genes,
      size.use = 0.05, size.title.use = 12, size.x.use = 12, size.y.use = 10, cols.use = color_scheme)
  dev.off()

  # FeaturePlot color scale (default: c("yellow", "red"))
  ...
  feat_filename_png = paste0(name, ".tsne.png")
  message("save plot: ", feat_filename_png)
  png(feat_filename_png, res = 200, width = 10, height = 8, units = "in")
    FeaturePlot(seurat_obj, genes, pt.size = 0.5, cols.use = featplot_colors, no.axes = TRUE, no.legend = TRUE)
  dev.off()
  feat_filename_pdf = paste0(name, ".tsne.pdf")
  message("save plot: ", feat_filename_pdf)
  pdf(feat_filename_pdf, width = 10, height = 8)
    FeaturePlot(seurat_obj, genes, pt.size = 0.5, cols.use = featplot_colors, no.axes = TRUE, no.legend = TRUE)
  dev.off()

  # expression levels per cluster for bar plots
  cluster_avg_exp = AverageExpression(seurat_obj)
  cluster_avg_exp_long = cluster_avg_exp %>% rownames_to_column("gene") %>% gather(cluster, avg_exp, -gene)
  cluster_avg_exp_subset = cluster_avg_exp_long %>% filter(gene %in% genes)

  # bar plots
  barplot_filename_png = paste0(name, ".barplot.png")
  barplot_plot = ggplot(cluster_avg_exp_subset, aes(x = cluster, y = avg_exp, fill = cluster)) +
    geom_col(color = "black") + theme(legend.position = "none") +
    scale_fill_manual(values = color_scheme) + scale_y_continuous(expand = c(0, 0)) +
    theme_cowplot() + facet_wrap(~ gene, ncol = 3, scales = "free")
  ggsave(barplot_filename_png, plot = barplot_plot, width = 10, height = 8, units = "in")
...
calculate_cluster_stats = function(seurat_obj, label) {
  ...
  # gene expression for an "average" single cell in each identity class (add add.ident for all cells)
...
calculate_cluster_markers = function(seurat_obj, label, test = "roc") {
  ...
  all_markers = FindAllMarkers(seurat_obj, thresh.use = 0.25, min.pct = 0.25, min.diff.pct = -Inf,
  ...
  if (test == "roc") {
    ...
    all_markers = all_markers %>% select(cluster, gene, avg_diff, myAUC, power) %>%
      filter(myAUC > 0.3 & power > 0.3) %>%
      mutate(avg_diff = round(avg_diff, 3), power = round(power, 3)) %>%
      arrange(cluster, -myAUC)
    top_markers = all_markers %>% group_by(cluster) %>% top_n(20, myAUC)
  } else {
    # bimod: likelihood-ratio test for single cell gene expression based on zero-inflated data
    # tobit: Tobit-test for differential gene expression as in Trapnell et al., Nature Biotech, 2014
    all_markers = all_markers %>% select(cluster, gene, p_val, avg_diff) %>%
      filter(p_val < 0.001 & abs(avg_diff) > 1) %>%
      mutate(avg_diff = round(avg_diff, 3)) %>%
      arrange(cluster, p_val)
    top_markers = all_markers %>% group_by(cluster) %>% top_n(20, abs(avg_diff))
  }

  all_markers_csv = paste0(markers_dir, "/markers.", label, ".", test, ".all.csv")
  message("all markers: ", all_markers_csv)
  write_excel_csv(all_markers, path = all_markers_csv)

  top_markers_csv = paste0(markers_dir, "/markers.", label, ".", test, ".top.csv")
  ...
  for (clust_name in clusters) {
    ...
    if (length(unique(clust_obj@ident)) > 1 && min(table(clust_obj@ident)) > 5) {
      ...
      for (combination_num in 1:ncol(group_combinations)) {
        ...
        de_genes = FindMarkers(clust_obj, ident.1 = s1, ident.2 = s2, test.use = "bimod", min.pct = 0.25, thresh.use = 0.25,
...
if (opts$create) {
  ...
  if (opts$identify || opts$diff) {
    ...
    res_label = paste0("clust", num_clusters)
    res_dir = paste0("clusters-res", res_str, "-", res_label)
    ...
    if (opts$identify) {
      ...
      tsne_filename = paste0("tsne.res", res_str, ".", res_label)
    ...
    num_samples = seurat_obj@data.info$orig.ident %>% unique() %>% as.character() %>% length()
    if (num_samples > 1) {

      if (opts$identify) {

        res_label = paste0(res_label, "-per-sample")
        seurat_obj@data.info$clust.sample = paste0(seurat_obj@ident[rownames(seurat_obj@data.info)], "-",
                                                   seurat_obj@data.info$orig.ident)
